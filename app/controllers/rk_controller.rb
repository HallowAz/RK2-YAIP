...
class RkController < ApplicationController
  ...
  def validate_alphabet
    flash[:notice] = 'Не должно быть букв' if @original_array.match?(/[A-Za-z]/).nil?
  end
  ...
  def validate_empty
    flash[:notice] = 'Массив не должен быть пустым' if @original_array.empty?
  end

  def validate_array
    flash[:notice] = 'Должны быть массив через пробел' if @original_array.match(/^[\d -]+$/).nil?
  end
